        ...
        const upload = (file, customConfig) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                imagekitProvider.upload({
                    ...
                    console.error('!!!!!!  UPLOAD IMAGE KIT FILE error !!!!!!', error);
        ...
        return {
            ...
            delete: file => {
                return new Promise(async (resolve, reject) => {
                    ...
                    let canDelete =  (uploadEnvironment === file.provider_metadata?.uploadEnvironment);
                    // Get provider metadata to check uploadEnvironment (if not, keep uploadEnvironment from API)
                    await imagekitProvider.getFileDetails(fileId).then(response => {
                        console.log('----- DELETE IMAGE KIT: file infos -----', fileId, response?.customMetadata);
                        canDelete = response?.customMetadata?.uploadEnvironment ? (uploadEnvironment === response.customMetadata.uploadEnvironment) : canDelete;
                    }).catch(error => {
                        console.error('!!!!!!  DELETE IMAGE KIT FILE error !!!!!!', error, fileId);
                        strapi.log.error(`Unable to delete file. ID:${fileId}`);
                        return reject(error);
                    })
                    if (file.provider_metadata?.avatar) {
                        canDelete = file.provider_metadata?.avatar;
                        console.warn('$$$$$$$ AVATAR CANNOT BE DELETED $$$$$$', fileId);
                    }
                    if (!canDelete) {
                    ...
                    console.log('----- DELETE IMAGE KIT: can delete -----', canDelete)
                    // Delete file
                    imagekitProvider.deleteFile(fileId).then(response => {
                        console.log('----- DELETE IMAGE KIT FILE OK -----', fileId);
